...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private func handleVideoSelectedForUrl (fileUrl: URL) {
        ...
        let uploadTask = Storage.storage().reference().child("message_movies").child(fileName).putFile(from: fileUrl, metadata: nil, completion: { (metdata, error) in
            if error != nil {
    ...
    private func uploadToFirebaseUsingSelectedImage(image: UIImage, completion: @escaping (_ imageUrl: String) -> ()) {
        ...
        if let uploadData = UIImageJPEGRepresentation(image, 0.2) {
           ...
           ref.putData(uploadData, metadata: nil, completion: { (metdata, error) in
                if error != nil {
                    print(error!)
                    return
                }
